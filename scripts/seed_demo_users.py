...
def seed_demo_users(db: Session) -> list[User]:
    ...
    email are left untouched (no history appended).
    """
    ...
    for spec in DEMO_USERS:
        ...
        if user is None:
            ...
            db.add(user)
            db.flush()
            logger.info("Created demo user", email=user.email, role=user.role)
        else:
            logger.info("Demo user already exists", email=user.email)
